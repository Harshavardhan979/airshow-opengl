...
float cloudX[5] = {-40, 40, 0, 30, -70};  // Initial cloud positions (X)
float cloudZ[5] = {0, 0, -70, 70, 70};   // Initial cloud positions (Z)
...
void cloud() {
    glPushMatrix();
    glScalef(1.5, 1, 1.25);
    glTranslatef(cloudX[0], 12, cloudZ[0]);
    sphere(1, 1, 1, 0.9);
    glPopMatrix();

    glPushMatrix();
    glScalef(1.5, 1, 1.25);
    glTranslatef(cloudX[1], 5, cloudZ[1]);
    sphere(1, 1, 1, 0.9);
    glPopMatrix();

    glPushMatrix();
    glScalef(1.5, 1, 1.25);
    glTranslatef(cloudX[2], 7, cloudZ[2]);
    sphere(1, 1, 1, 0.9);
    glPopMatrix();

    glPushMatrix();
    glScalef(1.5, 1, 1.25);
    glTranslatef(cloudX[3], 7, cloudZ[3]);
    sphere(1, 1, 1, 0.9);
    glPopMatrix();

    glPushMatrix();
    glScalef(2, 1.5, 2);
    glTranslatef(cloudX[4], 5, cloudZ[4]);
    sphere(1, 1, 1, 0.5);
    glPopMatrix();
}
// Global variables for cloud positions
float cloudPos1[3] = {-40.0, 10.0, 0.0};
float cloudPos2[3] = {40.0, 10.0, 0.0};
float cloudPos3[3] = {0.0, 20.0, -70.0};
float cloudPos4[3] = {30.0, 20.0, 70.0};
float cloudPos5[3] = {-70.0, 20.0, 70.0};

void moveClouds() {
    for (int i = 0; i < 5; i++) {
        cloudX[i] += 0.05f;  // Move clouds to the right
        if (cloudX[i] > 100) {
            cloudX[i] = -100;  // Reset position when cloud moves off-screen
        }
    }
}
...
void display1(void)
{
    ...
    // Clouds (with updated positions)
    glPushMatrix();
    glTranslatef(cloudPos1[0], cloudPos1[1], cloudPos1[2]);
    glScalef(2, 1.5, 2);
    cloud();
    glPopMatrix();

    glPushMatrix();
    glTranslatef(cloudPos2[0], cloudPos2[1], cloudPos2[2]);
    glScalef(1.5, 1, 1.5);
    cloud();
    glPopMatrix();

    glPushMatrix();
    glTranslatef(cloudPos3[0], cloudPos3[1], cloudPos3[2]);
    glRotatef(45, 0, 1, 0);
    glScalef(1, 1, 1);
    cloud();
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef(cloudPos4[0], cloudPos4[1], cloudPos4[2]);
    glRotatef(45, 1, 0, 0);
    glScalef(0.5, 0.5, 0.5);
    cloud();
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef(cloudPos5[0], cloudPos5[1], cloudPos5[2]);
    glRotatef(35, 0, 1, 0);
    glScalef(2, 1, 2);
    cloud();
    glPopMatrix();
...
void front()
{
	...
	output(250,650,"",fonts[2]);
	glColor3f(1.0,1.0,0.0);
	output(300,600,"GRAPHICAL IMPLEMENTATION OF AIRSHOW",fonts[2]);
	//glColor3f(0.8,0.1,0.2); red
	glColor3f(1.0,1.0,1.0);
	output(400,500,"GUIDE : Mr. Pakruddin B ",fonts[2]);
	glColor3f(0.8,0.1,0.2);
	output(450,400,"SUBMITTED BY :",fonts[2]);
	glColor3f(0.3,0.5,0.8);
	output(200,350,"1] MANOJ KUMAR BM",fonts[2]);
	output(650,350,"3] HARSHAVARDHAN K",fonts[2]);
	output(450,350,"2] MITUL SAI S",fonts[2]);
	output(400,290,"4] RAMKUMAR I ASKI",fonts[2]);
...
{
	glColor3f(0.8,0.1,0.2);
	output(170,600,"GRAPHICAL IMPLEMENTATION OF AIRSHOW    GUIDE =",fonts[2]);
	glColor3f(0.0,0.6,0.3);
